...
numfiles = 40;                          %Change from 1-40, leave at 40 if not testing
...
%  Then, fill it with the interpolated surface.
...
hold on
for i = 1:numfiles
   plot(mat(i).cudist(1).c ./ max(mat(i).cudist(1).c))
   
   for nmic = 2:10
        if length(m(i).m)>=nmic
            plot(mat(i).cudist(nmic).c ./ max(mat(i).cudist(nmic).c))
        end
    end
end
...
hold on
for i = 1:numfiles
    plot(mat(i).cudist(1).c)
    
    for nmic = 2:10
        if length(m(i).m)>=nmic
            plot(mat(i).cudist(nmic).c)
        end
    end
end
